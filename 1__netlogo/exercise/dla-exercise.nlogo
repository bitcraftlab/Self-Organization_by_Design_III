...
to setup
  ...
  create-stayers ( max-pxcor * 2 + 1 )
    [ 
      set ycor min-pycor
      set xcor (who  + min-pxcor )
      set color black 
    ]
  reset-ticks
end
...
to go
  ...
  ask stayers with [pycor >= max-pycor] [
    die
  ]
...
to make-mover
  create-movers 1 [
    ...
    set color gray
    set heading 0
    set xcor (min-pxcor + (random-float (max-pxcor - min-pxcor)))
    jump max-pxcor 
    ;; and point inward
    rt 180
    rt (random-float move-angle)
    set size 0.9
  ]
end
